...
enum class Roles(val code: String, val role: Role) {
  SuperUser("SUPER_USER", SuperUserRole), RegularUser("REGULAR_USER", RegularRole("REGULAR_USER", authorities {
    entityAccess(AclResourceIdentifier("account-aggregate")) {
      manageScope(AclResourceBelongsToAccountPredicate())
    }
  })),
  WorldCreator("WORLD_CREATOR", RegularRole("WORDL_CREATOR", authorities {
    entityAccess(AclResourceIdentifier("account-aggregate")) {
      manageScope(AclResourceBelongsToAccountPredicate())
    }
  }));
